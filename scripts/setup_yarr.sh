...
function compile_yarr {
  ...
  cmake3 -S $yarrDir -B $yarrDir/build -DCMAKE_BUILD_TYPE=Debug -DCMAKE_TOOLCHAIN_FILE=$yarrDir/cmake/linux-gcc -DYARR_CONTROLLERS_TO_BUILD="Spec;StarEmu;FelixClient" -DYARR_FRONT_ENDS_TO_BUILD=all -DBUILD_TESTS=on
  # or build all
  #cmake3 -S $yarrDir -B $yarrDir/build -DCMAKE_BUILD_TYPE=Debug -DCMAKE_TOOLCHAIN_FILE=$yarrDir/cmake/linux-gcc -DYARR_CONTROLLERS_TO_BUILD="Spec;Emu;StarEmu;Fei4Emu;NetioHW;FelixClient;ItsdaqFW" -DYARR_FRONT_ENDS_TO_BUILD=all -DBUILD_TESTS=on
